...
class DataIngestion:
    ...
    def split_data_as_train_test(self) -> DataIngestionArtifact:
        try:
            ...
            file_name = os.listdir(raw_data_dir)[0]    ## we are taking/copying file name inside a folder raw_data_dir (1st index of that list output)

            housing_file_path = os.path.join(raw_data_dir,file_name)  ## creating file path
 

            logging.info(f"Reading csv file: [{housing_file_path}]")
            housing_data_frame = pd.read_csv(housing_file_path)       ## reading file path

            housing_data_frame["income_cat"] = pd.cut(           
                housing_data_frame["median_income"],       ## stratified state means distribution of test and train dataset should align during splits
